...
- 模块 1 先做认证闭环，模块 2 接入帖子流和发帖，模块 3 接入帖子详情、点赞和收藏，模块 4 接入评论列表和回复，模块 5 接入评论点赞和删除，模块 6 接入用户公开主页，模块 7 接入关注关系。
...
27. 退出登录后确认帖子列表、详情、评论和用户主页仍可查看，但发布、点赞、收藏、评论、回复、关注按钮不可用。
