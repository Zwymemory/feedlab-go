...
FeedLab 是一个面向 Go 后端实习展示的内容社区系统。当前按小模块迭代实现 V1，已完成：
...
- Redis 在 V1 只做连接和健康检查，不设计业务 Key；帖子详情缓存、用户缓存、排行榜等留到 V3。
- V1 暂不接 RabbitMQ，因此没有队列、生产者、消费者和消息格式；通知异步化留到 V4。
...
当前模块不创建业务 Redis Key，只在启动和 `/healthz` 中执行 `PING`。后续 V3 会引入：
...
V1 不接 RabbitMQ，因此当前没有队列、生产者、消费者和消息格式。V4 再设计 `notification.queue`，由点赞、评论、关注接口生产通知消息，Worker 消费后写入 `notifications` 表。
