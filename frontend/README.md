...
- 评论点赞、取消点赞、点赞状态：`POST/DELETE/GET /api/v1/comments/:id/like(d)`。
- 删除评论或回复：`DELETE /api/v1/comments/:id`。
- 用户公开主页：`GET /api/v1/users/:id`。
- 用户公开帖子：`GET /api/v1/users/:id/posts?page=1&page_size=10`。
- 关注、取消关注、关注状态：`POST/DELETE/GET /api/v1/users/:id/follow(ed)`。
- 粉丝列表和关注列表：`GET /api/v1/users/:id/followers|following`。
- 用户点赞帖子列表和收藏帖子列表：`GET /api/v1/users/:id/likes|collects`。
...
- 模块 1 先做认证闭环，模块 2 接入帖子流和发帖，模块 3 接入帖子详情、点赞和收藏，模块 4 接入评论列表和回复，模块 5 接入评论点赞和删除，模块 6 接入用户公开主页，模块 7 接入关注关系，模块 8 接入用户点赞/收藏列表。
...
- 评论点赞是幂等接口，前端会按 `comment_id` 保存当前用户是否点过赞，并把返回的 `like_count` 同步回评论或回复。
- 删除评论只对评论作者或 `admin` 开放；删除一级评论时后端会级联软删除它的回复，前端按 `deleted_count` 同步帖子评论数。
- 用户公开主页使用公开 VO，不展示 `email`、`role`、`status` 等内部字段；任何人都可以查看公开资料和公开帖子。
- 作者名是用户主页入口：帖子卡片、帖子详情和评论作者名都可以打开对应用户主页。
- 关注和取消关注必须登录，因为它们表达的是“当前用户和目标用户”的关系；前端只负责按钮状态，后端仍用 JWT 判断真正的当前用户。
- 粉丝列表和关注列表是公开读取接口，只展示公开用户资料，不暴露邮箱、角色和状态等内部字段。
- 关注接口是幂等接口，重复关注不会重复增加粉丝数，重复取消关注也不会继续扣减；前端收到后端返回的 `follower_count` 后同步更新页面。
- 自己不能关注自己：前端在个人主页按钮上显示“这是你自己”，后端也会兜底校验，避免绕过页面直接请求 API。
- 用户主页中的“发帖 / 点赞 / 收藏”都是公开帖子列表，只展示 `published` 帖子；这样不会因为点赞或收藏泄露草稿、已删除内容。
...
16. 点击评论或回复的“点赞”，确认按钮变为“取消点赞”，点赞数增加。
17. 再次点击“取消点赞”，确认点赞数回退且不会重复扣减。
18. 对自己发布的回复点击“删除”，确认回复从列表消失，帖子评论数减少。
19. 对自己发布的一级评论点击“删除”，确认该评论和它已加载的回复一起从前端消失。
20. 点击帖子卡片、帖子详情或评论里的作者名，确认用户公开主页打开。
21. 在“用户公开主页”里输入用户 ID 并点击“打开主页”，确认能查看该用户资料和公开帖子列表。
22. 点击用户公开帖子列表中的“查看”，确认能打开对应帖子详情。
23. 打开非当前登录用户的主页，点击“关注”，确认按钮变为“取消关注”，粉丝数增加。
24. 再次点击“取消关注”，确认按钮回到“关注”，粉丝数回退且不会变成负数。
25. 在用户主页切换“粉丝 / 关注”，确认公开用户列表可以加载，点击列表里的用户可以继续打开他的主页。
26. 打开当前登录用户自己的主页，确认按钮显示“这是你自己”，并且不可点击。
27. 在用户主页切换“发帖 / 点赞 / 收藏”，确认可以查看该用户发布、点赞和收藏过的公开帖子。
28. 点击点赞列表或收藏列表中的“查看”，确认可以打开对应帖子详情。
29. 退出登录后确认帖子列表、详情、评论和用户主页仍可查看，但发布、点赞、收藏、评论、回复、关注按钮不可用。
