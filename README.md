...
当前已完成模块不创建业务 Redis Key，只在启动和 `/healthz` 中执行 `PING`。V2 点赞状态、收藏状态和评论列表先查 MySQL，后续 V3 会引入：
...
当前不接 RabbitMQ，因此没有队列、生产者、消费者和消息格式。V2 点赞、评论和收藏成功后暂不发送通知；V4 再设计 `notification.queue`，由点赞、评论、收藏、关注接口生产通知消息，Worker 消费后写入 `notifications` 表。
