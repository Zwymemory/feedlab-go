...
FeedLab 是一个面向 Go 后端实习展示的内容社区系统。当前按小模块迭代，V1 基础业务闭环、V2 互动系统、V3 Redis 缓存和 Feed 优化、V4 RabbitMQ 异步通知、V5 展示型前端均已完成：
...
- V3 模块 4：Feed 游标分页。
- V3 模块 5：Redis 帖子浏览量计数。
- V3 模块 6：Redis 评论列表缓存。
- V3 模块 7：Redis 缓存观测接口。
...
V3 完成后，如果你想把 Redis 缓存、排行榜、浏览量计数、限流、防穿透和观测接口串起来讲清楚，可以阅读：

[FeedLab Go V3 Redis 与缓存系统代码导读](./docs/feedlab-v3-redis-cache-guide.md)
...
- `page/page_size` 适合后台列表和简单翻页，但信息流滚动到很深的位置时，数据库需要跳过越来越多的记录，性能会变差。
- Feed 场景更适合游标分页：上一页返回 `next_cursor`，下一页带着它继续查更早的帖子，客户端不需要知道游标内部格式。
- 当前游标由上一页最后一条帖子的 `created_at` 和 `id` 组成，查询时使用 `(created_at DESC, id DESC)` 保持稳定顺序。
- `id` 是同一创建时间下的兜底排序字段，可以避免相同时间的帖子在翻页边界出现重复或漏查。
- 本模块不新增 Redis Key，也不新增 RabbitMQ 队列；它优化的是 MySQL 查询形态。后续如果做个性化 Feed 或推荐流，可以再考虑 Feed 缓存。

### GET /api/v1/feed/posts

用途：按游标分页查看 published 帖子流，适合前端无限滚动。

第一页测试：

```bash
curl 'http://localhost:8080/api/v1/feed/posts?limit=10'
```

下一页测试：

```bash
NEXT_CURSOR="上一步响应 data.next_cursor"

curl "http://localhost:8080/api/v1/feed/posts?limit=10&cursor=${NEXT_CURSOR}"
```

预期：

- 返回 `200`，`data.items` 是当前页帖子列表。
- `data.next_cursor` 是下一页游标；如果没有更多数据则为空字符串。
- `data.has_more` 表示是否还有下一页。
- `data.limit` 表示本次请求使用的分页大小，默认 `10`，最大 `50`。
- 非法 `cursor` 或非法 `limit` 返回 `400`，`code = 40000`。

## V3 模块 5：Redis 帖子浏览量计数

### 为什么这样设计

- 帖子详情是高频读接口，如果每次访问都直接 `UPDATE posts SET view_count = view_count + 1`，热门帖子会给 MySQL 带来大量写压力。
- V3 使用 Redis 先承接浏览增量：每次访问 `GET /api/v1/posts/:id` 时，对 `post:view_count:{post_id}` 执行 `INCR`，响应里的 `view_count` 会叠加 Redis 中的实时增量。
- 当 Redis 增量达到 `redis.post_view_flush_threshold` 阈值时，后端会把这批增量一次性写回 MySQL，并删除旧帖子详情缓存。
- 这种设计是最终一致性：浏览量不是强一致计数，允许短时间保存在 Redis；MySQL 仍然是最终数据源。
- 删除帖子时会删除对应的浏览量 Redis Key，避免已软删除内容继续保留临时浏览增量。

### Redis Key

| Key | 用途 | 过期策略 |
|---|---|---|
| `post:view_count:{post_id}` | 缓存某篇帖子的待落库浏览量增量 | 默认 TTL 86400 秒，可通过 `redis.post_view_ttl_seconds` 配置；达到 `redis.post_view_flush_threshold` 后批量落库并删除 |

### GET /api/v1/posts/:id 浏览量测试

用途：验证访问帖子详情时，Redis 浏览量增量会增加，并叠加到接口响应里。
...
redis-cli DEL "post:view_count:${POST_ID}"

curl "http://localhost:8080/api/v1/posts/${POST_ID}"
redis-cli GET "post:view_count:${POST_ID}"
redis-cli TTL "post:view_count:${POST_ID}"
...
- 第一次详情请求返回 `code = 0`，`post:view_count:{post_id}` 变为 `1`。
- 第二次详情请求后，该 Key 变为 `2`。
- `TTL` 返回大于 `0` 的秒数，默认接近 `86400`。
- 当增量达到 `redis.post_view_flush_threshold` 后，后端会把浏览量批量写入 MySQL，并删除 `post:detail:{post_id}`，让后续详情缓存重新生成。

## V3 模块 6：Redis 评论列表缓存

### 为什么这样设计

- 帖子评论列表和回复列表是高频读接口，热门帖子下同一页评论会被反复访问，适合用 Redis 缓存短时间结果。
- V3 缓存的是 `CommentList` VO，而不是 GORM Model，避免把数据库内部字段或软删除字段泄露到接口层。
- 评论列表是分页接口，所以 Redis Key 必须带上 `page` 和 `page_size`，否则不同分页请求会互相污染。
- 创建评论、删除评论、评论点赞和取消点赞都会改变列表可见内容或 `like_count`，因此这些写操作成功后要删除相关列表缓存。
- 列表缓存失效使用前缀扫描删除同一帖子或同一父评论下的所有分页 Key；这是 V3 简化方案，后续流量变大时可以考虑版本号 Key 或更细的分页失效策略。

### Redis Key

| Key | 用途 | 过期策略 |
|---|---|---|
| `post:comments:{post_id}:page:{page}:size:{page_size}` | 缓存某篇帖子的一级评论分页列表 | 默认 TTL 120 秒，可通过 `redis.comment_list_ttl_seconds` 配置；评论创建/删除/点赞变化后删除相关分页 Key |
| `comment:replies:{comment_id}:page:{page}:size:{page_size}` | 缓存某条一级评论的二级回复分页列表 | 默认 TTL 120 秒，可通过 `redis.comment_list_ttl_seconds` 配置；回复创建/删除/点赞变化后删除相关分页 Key |

### GET /api/v1/posts/:id/comments 缓存测试

用途：验证一级评论列表第一次从 MySQL 回源，第二次从 Redis 命中。
...
redis-cli DEL "post:comments:${POST_ID}:page:1:size:10"

curl "http://localhost:8080/api/v1/posts/${POST_ID}/comments?page=1&page_size=10"
redis-cli TTL "post:comments:${POST_ID}:page:1:size:10"
redis-cli GET "post:comments:${POST_ID}:page:1:size:10"
...
- 第一次请求返回 `code = 0`，并写入 `post:comments:{post_id}:page:1:size:10`。
- `TTL` 返回大于 `0` 的秒数，默认接近 `120`。
- 第二次请求返回同样结构，理论上命中 Redis。
...
curl "http://localhost:8080/api/v1/posts/${POST_ID}/comments?page=1&page_size=10"
redis-cli EXISTS "post:comments:${POST_ID}:page:1:size:10"

curl -X POST "http://localhost:8080/api/v1/posts/${POST_ID}/comments" \
  -H "Authorization: Bearer ${TOKEN}" \
  -H 'Content-Type: application/json' \
  -d '{"content":"触发评论列表缓存失效","parent_id":0}'

redis-cli EXISTS "post:comments:${POST_ID}:page:1:size:10"
```

预期：发布一级评论成功后 `EXISTS` 返回 `0`，表示旧评论列表缓存已失效。

### GET /api/v1/comments/:id/replies 缓存测试
...
```bash
COMMENT_ID="已有一级评论 ID"

redis-cli DEL "comment:replies:${COMMENT_ID}:page:1:size:10"

curl "http://localhost:8080/api/v1/comments/${COMMENT_ID}/replies?page=1&page_size=10"
redis-cli TTL "comment:replies:${COMMENT_ID}:page:1:size:10"
redis-cli GET "comment:replies:${COMMENT_ID}:page:1:size:10"
```

预期：请求成功后写入 `comment:replies:{comment_id}:page:1:size:10`，TTL 默认接近 `120`。

## V3 模块 7：Redis 缓存观测接口

### 为什么这样设计

- V3 已经引入多个 Redis Key，如果只能靠 `redis-cli` 手动查，调试成本会变高，也不利于向面试官展示缓存是否真的工作。
- 本模块提供一个登录后可用的诊断接口，只返回 Key 是否存在、TTL、浏览量增量和热门榜分数，不返回缓存正文，避免调试接口泄露业务内容。
- 观测接口放在 Controller-Service 分层里：Controller 解析 HTTP 参数，Service 读取 Redis 状态，不绕过统一响应。
- 这个接口不修改业务数据，不需要事务；它只是读 Redis 状态，辅助本地验证和排障。
- 当前只面向帖子相关缓存，后续如果引入更多 Redis Key，可以继续扩展同一类诊断接口。

### GET /api/v1/cache/posts/:id/status

用途：查看某篇帖子相关 Redis 缓存状态。需要登录。
...
curl "http://localhost:8080/api/v1/cache/posts/${POST_ID}/status?page=1&page_size=10" \
  -H "Authorization: Bearer ${TOKEN}"
```
...
```json
{
  ...
  "data": {
    ...
    "post_detail_null": {
      "key": "post:detail:null:1",
      "exists": false,
      "ttl_seconds": -2
    },
    "post_view_count": {
      "key": "post:view_count:1",
      "exists": true,
      "ttl_seconds": 86390,
      "value": 3
    },
    "comments": {
      "key": "post:comments:1:page:1:size:10",
      "exists": true,
      "ttl_seconds": 110
    },
    "hot_rank": {
...
- `ttl_seconds = -2`：Key 不存在。
- `ttl_seconds = -1`：Key 存在但没有过期时间，例如 `rank:hot_posts` 这类不设置 TTL 的结构。
- `post_view_count.value`：当前 Redis 中尚未批量落库的浏览量增量。
- `comments.key`：由请求里的 `page` 和 `page_size` 决定，用于检查对应评论分页缓存。

## V3 模块 8：Redis 空值缓存防穿透

### 为什么这样设计

- 如果大量请求反复访问不存在的帖子 ID 或用户 ID，普通缓存查不到数据，会每次都回源 MySQL，这叫缓存穿透。
- V3 对帖子详情和用户公开资料增加短 TTL 空值缓存：当 MySQL 确认资源不存在时，写入一个只表示“不存在”的 Redis Key。
- 后续同一个不存在 ID 再次请求时，Service 层命中空值缓存后直接返回 `40400`，不再打 MySQL。
- 空值缓存 TTL 默认只有 60 秒，避免长期挡住未来可能创建的数据，也降低误判影响。
- 空值缓存只保存 `"1"` 这种哨兵值，不保存业务正文；删除对应业务缓存时，也会同时删除空值缓存。

### Redis Key

| Key | 用途 | 过期策略 |
|---|---|---|
| `post:detail:null:{post_id}` | 记录某个帖子详情刚刚确认不存在或不可公开访问 | 默认 TTL 60 秒，可通过 `redis.null_cache_ttl_seconds` 配置 |
| `user:profile:null:{user_id}` | 记录某个用户公开资料刚刚确认不存在 | 默认 TTL 60 秒，可通过 `redis.null_cache_ttl_seconds` 配置 |

### 帖子详情空值缓存测试

用途：验证不存在的帖子 ID 第一次回源 MySQL 后写入空值缓存，第二次直接被空值缓存挡住。

测试：

```bash
MISSING_POST_ID="99999999"

redis-cli DEL "post:detail:null:${MISSING_POST_ID}"

curl "http://localhost:8080/api/v1/posts/${MISSING_POST_ID}"
redis-cli TTL "post:detail:null:${MISSING_POST_ID}"
redis-cli GET "post:detail:null:${MISSING_POST_ID}"
...
- 两次接口都返回 `404`，`code = 40400`。
- 第一次请求后写入 `post:detail:null:{post_id}`。
- `TTL` 返回大于 `0` 的秒数，默认接近 `60`。

### 用户公开资料空值缓存测试

用途：验证不存在的用户 ID 会写入 `user:profile:null:{user_id}`。
...
redis-cli DEL "user:profile:null:${MISSING_USER_ID}"

curl "http://localhost:8080/api/v1/users/${MISSING_USER_ID}"
redis-cli TTL "user:profile:null:${MISSING_USER_ID}"
redis-cli GET "user:profile:null:${MISSING_USER_ID}"
```

预期：接口返回 `40400`，Redis 中写入短 TTL 空值缓存。

## V3 模块 9：Redis 登录限流

### 为什么这样设计

- 登录接口是账号系统最敏感的入口，如果不做限制，攻击者可以高频尝试邮箱和密码组合，形成暴力破解风险。
- V3 使用 Gin 中间件保护 `POST /api/v1/auth/login`，用 Redis `INCR + EXPIRE` 做固定窗口限流。
- Key 按客户端 IP 维度生成：`rate_limit:login:{ip}`。同一个 IP 在一个窗口内超过配置次数，就直接返回 `429`。
- Redis 只做限流计数，不保存密码、Token 或请求体内容。
- Redis 不可用时，中间件降级放行，避免缓存故障导致所有用户无法登录；真实生产环境可以按安全策略改成 fail-closed。

### Redis Key

| Key | 用途 | 过期策略 |
|---|---|---|
| `rate_limit:login:{ip}` | 记录某个 IP 在当前窗口内尝试登录的次数 | 默认窗口 60 秒，可通过 `rate_limit.login_window_seconds` 配置；超过 `rate_limit.login_max_attempts` 后返回 429 |

### POST /api/v1/auth/login 限流测试

用途：验证登录接口在一个窗口内超过次数后返回 429。
...
for i in {1..11}; do
  curl -s -o /dev/null -w "%{http_code}\n" \
    ...
    -H 'Content-Type: application/json' \
    -d '{"email":"not-exist@example.com","password":"wrong-password"}'
done
```

预期：

- 默认 `rate_limit.login_max_attempts = 10`，前 10 次通常返回 `401` 或业务错误。
- 第 11 次返回 HTTP `429`，统一响应 `code = 42900`。
- 可用下面命令查看限流 Key：

```bash
redis-cli KEYS 'rate_limit:login:*'
redis-cli TTL 'rate_limit:login:127.0.0.1'
redis-cli GET 'rate_limit:login:127.0.0.1'
```

注意：如果本机请求 IP 不是 `127.0.0.1`，请以 `KEYS` 查到的实际 Key 为准。

## V3 收尾：Redis 与缓存系统代码导读

V3 已经覆盖了内容社区后端常见的 Redis 能力：

- Cache Aside：帖子详情、用户公开资料、评论列表。
- ZSet 排行榜：热门帖子 Top N。
- INCR 计数：浏览量待落库增量、登录限流。
- 空值缓存：防止不存在 ID 反复穿透 MySQL。
- 缓存观测：查看 Key 是否存在、TTL、计数增量和热门榜分数。
...
- Redis 和 MySQL 谁是最终数据源？
- 为什么写操作后多数选择删除缓存，而不是直接更新缓存？
- 哪些 Key 有 TTL，哪些 Key 没有 TTL？
- 浏览量为什么可以最终一致，哪些业务不能这样做？
- 空值缓存解决什么问题？为什么 TTL 要短？
- 登录限流为什么适合做成中间件？
...
- `post:view_count:{post_id}`：帖子浏览量待落库增量，TTL 默认 86400 秒；达到阈值后批量写回 MySQL 并删除该 Key。
- `post:comments:{post_id}:page:{page}:size:{page_size}`：一级评论分页列表缓存，TTL 默认 120 秒；评论创建、删除或点赞变化后删除相关分页 Key。
- `comment:replies:{comment_id}:page:{page}:size:{page_size}`：二级回复分页列表缓存，TTL 默认 120 秒；回复创建、删除或点赞变化后删除相关分页 Key。
- `post:detail:null:{post_id}`：帖子详情空值缓存，TTL 默认 60 秒；用于防止不存在帖子 ID 反复穿透到 MySQL。
- `user:profile:null:{user_id}`：用户公开资料空值缓存，TTL 默认 60 秒；用于防止不存在用户 ID 反复穿透到 MySQL。
- `rate_limit:login:{ip}`：登录限流计数，默认窗口 60 秒；同一 IP 超过默认 10 次后返回 429。

`GET /api/v1/feed/posts` 当前不新增 Redis Key。它通过游标分页优化 MySQL 查询方式，后续如果要做个性化推荐流，再单独设计 Feed 缓存 Key。

`GET /api/v1/cache/posts/:id/status` 当前不新增业务 Redis Key。它读取并返回帖子相关 Key 的存在状态、TTL、浏览量增量和热门榜分数，用于本地验证和排障。
